...
const meLimiter = limiterPorUsuario(60 * 60 * 1000, 10);
...
app.get("/livros/:slug", async (req: Request, res: Response) => {
...
app.get("/livros/:slug/capitulos/:capSlug", async (req: Request, res: Response) => {
  ...
  res.json({ ...capitulo, livro: { slug: livro.slug, title: livro.title } });
});
...
app.post("/cadastro", cadastroLimiter, validate(cadastroSchema), async (req: Request, res: Response) => {
...
app.post("/login", loginLimiter, validate(loginSchema), async (req: Request, res: Response) => {
  ...
  const accessToken = jwt.sign({ id: usuario.id, email: usuario.email }, JWT_SECRET, { expiresIn: "15m" });
...
app.post("/refresh", refreshLimiter, async (req: Request, res: Response) => {
  ...
  const novoAccessToken = jwt.sign({ id: usuario.id, email: usuario.email }, JWT_SECRET, { expiresIn: "15m" });
...
app.get("/admin/me", requireAdmin, (req: Request, res: Response) => {
  ...
  res.json({ id: usuario.id, nome: usuario.nome, email: usuario.email, isAdmin: usuario.isAdmin, bio: usuario.bio });
});
...
app.post(
  "/admin/livros/:slug/capitulos",
  doubleCsrfProtection,
  requireAdmin,
  validate(capituloSchema),
  async (req: Request, res: Response) => {
